...
<div class="card card-secondary">
    <div class="card-header">
        <h3 class="card-title">Lista de Anticipos de Proveedores</h3>
        <div class="float-right">
          
            <a class="btn btn-info btn-sm" href="{{ asset('admin/archivos/FORMATO_ANTICIPO_PROVEEDOR.xlsx') }}" download="FORMATO ANTICIPO"><i class="fas fa-file-excel"></i>&nbsp;Formato</a>
            <a class="btn btn-success btn-sm" href="{{ url("excelAnticipoProveedor") }}"><i class="fas fa-file-excel"></i>&nbsp;Cargar Excel</a>
        </div>
    </div>
    <!-- /.card-header -->
    <div class="card-body">
        <form class="form-horizontal" method="POST" action="{{ url("listaAnticipoProveedor") }}">
        @csrf
                    ...
                    @if(isset($datos))
                        @for ($i = 1; $i <= count($datos); ++$i)   
                            <tr>
                                ...
                                @if(Auth::user()->empresa->empresa_llevaContabilidad == '1')
                                <td class="text-center" @if($datos[$i]['tot'] == '1') style="background:  #70B1F7;" @endif @if($datos[$i]['tot'] == '2') style="background:  #C0C4C5;" @endif><a href="{{ url("asientoDiario/ver/{$datos[$i]['dir']}") }}" target="_blank">{{ $datos[$i]['dir'] }}</a> </td>
                                @endif       
                                <td @if($datos[$i]['tot'] == '1') style="background:  #70B1F7;" @endif @if($datos[$i]['tot'] == '2') style="background:  #C0C4C5;" @endif>{{ $datos[$i]['tip'] }} <input type="hidden" name="idTip[]" value="{{ $datos[$i]['tip'] }}"/><input type="hidden" name="idDir[]" value="{{ $datos[$i]['dir'] }}"/></td>
