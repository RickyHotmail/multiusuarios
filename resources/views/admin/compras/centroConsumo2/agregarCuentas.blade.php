...
@section('principal')
<form class="form-horizontal" method="POST" action="{{ url('centroConsumo2') }}/{{ $cuentaPadre->centroc2_id }}/agregarSubCuenta">
@csrf
    <div class="card card-secondary">
        ...
        <div class="card-body">
            <div class="form-group row">
                ...
                <div class="col-sm-9">
                    <input type="hidden"id="cuenta_nivel" name="cuenta_nivel" value="{{$cuentaPadre->centroc2_nivel + 1 }}">
                    <label class="form-control">{{$cuentaPadre->centroc2_nivel + 1 }}</label>
                </div>
            </div>
            <div class="form-group row">
                <label for="cuenta_numero" class="col-sm-3 col-form-label">Numero</label>
                <div class="col-sm-3">
                    <input type="hidden" id="cuenta_padre" name="cuenta_padre" value="{{$cuentaPadre->centroc2_secuencial}}"/>
                    <label class="form-control">{{$cuentaPadre->centroc2_secuencial.'.'}}</label>
                </div>
                <div class="col-sm-6">
