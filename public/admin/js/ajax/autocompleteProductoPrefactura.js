		...
		$("#buscarProducto").autocomplete({
			source: function(request, response){
				if($("#clienteID").val() != ''){
					...
					$.ajax({
						async: false,
						url: contextPath+"/productoVentaPrefactura/searchN/"+request.term,
						dataType: "json",
						type: "GET",
						data: {
							buscar: request.term,
				...
				if(parseFloat(ui.item.stock) > 0 || ui.item.tipop == '2'|| ui.item.cv == '2'){
					...
					$.ajax({
						...
						data: {
							...
							tipoPago: $("#factura_tipo_pago").val(),
							plazo: $("#factura_dias_plazo").val(),
						},
						success: function(data){
							$("#id_pu").val(data); 
						},
					});
					if($("#id_pu").val() == 0.00 && ui.item.cv == '3' && ui.item.tipop != '2'){
						bootbox.alert({
