...
class Nota_Credito_banco extends Model
{
    ...
    public function scopeNotasCreditoBancos($query){
        return $query->join('rango_documento','rango_documento.rango_id','=','nota_credito_banco.rango_id')
    ...
    public function scopeNCbancoByCuenta($query, $id){
        return $query->join('rango_documento','rango_documento.rango_id','=','nota_credito_banco.rango_id')
                    ...
                    ->join('sucursal','sucursal.sucursal_id','=','punto_emision.sucursal_id')
                    ->where('sucursal.empresa_id','=',Auth::user()->empresa_id)
                    ->where('nota_credito_banco.cuenta_bancaria_id','=',$id)
                    ->orderby('nota_credito_banco.nota_fecha','asc');
    }
