...
class Anticipo_Cliente extends Model
{
    ...
    public function scopeAnticipos($query){
        return $query->join('rango_documento','rango_documento.rango_id','=','anticipo_cliente.rango_id')
                    ...
                    ->orderBy('anticipo_fecha','asc');
    }
    public function scopeAnticipoDiario($query,$diario){
        return $query->join('diario', 'diario.diario_id','=','anticipo_cliente.diario_id')
                    ->where('diario.empresa_id','=',Auth::user()->empresa_id)
                    ->where('anticipo_cliente.diario_id','=',$diario)
                    ->orderBy('anticipo_fecha','asc');
    }
    public function scopeAnticipoClienteByFechaSucursal($query,$cliente, $sucursal,$desde, $hasta){
        ...
        if($sucursal!='0') $query->where('punto_emision.sucursal_id', '=', $sucursal);
        return $query;
    }
    ...
    public function scopeAnticiposByCliente($query, $cliente_id){
        return $query->join('rango_documento','rango_documento.rango_id','=','anticipo_cliente.rango_id')
                    ->join('punto_emision','punto_emision.punto_id','=','rango_documento.punto_id')
                    ->join('sucursal','sucursal.sucursal_id','=','punto_emision.sucursal_id')
                    ->join('diario','diario.diario_id','=','anticipo_cliente.diario_id')
                    ->where('sucursal.empresa_id','=',Auth::user()->empresa_id)
    ...
    public function scopeAntCliByFec($query, $fechaI, $fechaF, $cliente_id, $sucursal_id,$todo){
        $query->join('rango_documento','rango_documento.rango_id','=','anticipo_cliente.rango_id')
            ...
            ->join('sucursal','sucursal.sucursal_id','=','punto_emision.sucursal_id')
            ->where('sucursal.empresa_id','=',Auth::user()->empresa_id)
            ->where('cliente_id','=',$cliente_id)->orderBy('anticipo_fecha','asc');
    
        if($sucursal_id != '0') $query->where('sucursal.sucursal_id','=',$sucursal_id);
        if($todo != 1) $query->where('anticipo_fecha','>=',$fechaI)->where('anticipo_fecha','<=',$fechaF);
    ...
    public function scopeAnticiposByClienteFecha($query, $cliente_id, $fecha){
        return $query->join('rango_documento','rango_documento.rango_id','=','anticipo_cliente.rango_id')
                    ...
                    ->where('sucursal.empresa_id','=',Auth::user()->empresa_id)
                    ->where('cliente_id','=',$cliente_id)
                    ->where('anticipo_fecha','<=',$fecha)
                    ->orderBy('anticipo_fecha','asc');
    }
    public function scopeAnticipoClienteDescuentos($query, $id){
        return $query->join('descuento_anticipo_cliente','descuento_anticipo_cliente.anticipo_id','=','anticipo_cliente.anticipo_id')
                    ->where('anticipo_cliente.anticipo_id','=',$id);
    }
