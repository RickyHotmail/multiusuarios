...
class facturacionElectronicaController extends Controller
{
    ...
    function firmarDoc($archivo, $firmaElectronica){
        ...
        if($firmaElectronica->firma_ambiente==1)
            $ruta = public_path().'/documentosElectronicos/'.$empresa->empresa_ruc.'/XML_FIRMADOS';
        else
            $ruta = public_path().'/documentosElectronicos/'.$empresa->empresa_ruc.'/PRUEBAS/XML_FIRMADOS';

        if (!is_dir($ruta)) mkdir($ruta, 0777, true);
        
        //$signedXML = xmlwriter_output_memory($signedXML);
        $nombreArchivo = Date('d-m-Y-h-i-s').".xml";
        $archivo = fopen($ruta.'/'.$nombreArchivo, 'w');
        fwrite($archivo, $combprobanteFirmado);
        fclose($archivo);
